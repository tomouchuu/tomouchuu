...
export const personal = new Elysia({ prefix: "/personal" }).get("/", async () => {
  ...
  const image = `https://cdn.discordapp.com/avatars/${discordId}/${discordPresenceData.data.discord_user.avatar}.webp?size=256`;

  return {
    name: "Thomas Moore",
    image,
    status: discordPresenceData.data.discord_status,
    location: `Currently in ${discordPresenceData.data.kv.location}`,
    contact: {
